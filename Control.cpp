...
uLCD_4DGL::uLCD_4DGL(PinName tx, PinName rx, PinName rst) :
    cmd_(tx, rx, 9600),
    rst_(rst)
#if DEBUGMODE
    ,
...
#endif // DEBUGMODE
{
  ...
  current_hf          = 1;
  current_wf          = 1;
  // not needed, always use text_string
  // set_font(FONT_7X8);                 // initial font
  //   text_mode(OPAQUE);                  // initial texr mode
}

void
uLCD_4DGL::writeBYTE(char c) // send a BYTE command to screen
{
  // have to have buffer, no putc
  char buffer[1] = "";
  buffer[0]      = c;
  cmd_.write(buffer, 1);
  // CHANGED from 500us to 1ms, doesnt support us
  ThisThread::sleep_for(
    1ms); // mbed is too fast for LCD at high baud rates in some long commands
...
void
uLCD_4DGL::writeBYTEfast(char c) // send a BYTE command to screen
{
  // have to have buffer, no putc
  char buffer[1] = "";
  buffer[0]      = c;
  cmd_.write(buffer, 1);
  // wait_ms(0.0);  //mbed is too fast for LCD at high baud rates - but not in
  // short commands
...
void
uLCD_4DGL::freeBUFFER(void) // Clear serial buffer before writing command
{
  char* buffer = 0;
  // replace getc with read
  while (cmd_.readable())
    cmd_.read(buffer, 1); // clear buffer garbage
}

int
uLCD_4DGL::writeCOMMAND(
  char* command,
  int   number) // send several BYTES making a command and return an answer
{
...
void
uLCD_4DGL::reset() // Reset Screen
{
  ThisThread::sleep_for(5ms);
  rst_ = 0;                   // put RESET pin to low
  ThisThread::sleep_for(5ms); // wait a few milliseconds for command reception
  rst_ = 1;                   // put RESET back to high
  ThisThread::sleep_for(3s);  // wait 3s for screen to restart

  freeBUFFER(); // clean buffer from possible garbage
}

int
uLCD_4DGL::writeCOMMANDnull(
  char* command,
  int   number) // send several BYTES making a command and return an answer
{
...
int
uLCD_4DGL::version() // get API version
{
...
void
uLCD_4DGL::baudrate(int speed) // set screen baud rate
{
  char command[3] = "";
  writeBYTE(0x00);
  command[0]  = BAUDRATE;
  ...
  // cmd_.set_baud(speed);   not allowed                            // set mbed
  // to same speed
  i = 0;
  while ((!cmd_.readable()) && (i < 25000)) {
    ThisThread::sleep_for(
      0s); // wait for screen answer - comes 100ms after change
    i++;   // timeout if ack character missed by baud change
  }
  char* ret = 0;
  if (cmd_.readable())
    cmd_.read(ret, 1); // read response if any
  resp = (int)*ret;    // cast response to int
  switch (resp) {
...
int
uLCD_4DGL::readVERSION(
  char* command,
  int   number) // read screen info and populate data
{
  ...
  while (cmd_.readable() && (unsigned int)resp < ARRAY_SIZE(response)) {
    // change from getc
    char* ret = 0;
    cmd_.read(ret, 1);
    temp             = (int)*ret;
...
void
uLCD_4DGL::background_color(int color) // set screen background color
{
  ...
  command[1] =
    ((red5 << 3) + (green6 >> 3)) & 0xFF; // first part of 16 bits color
  command[2] =
    ((green6 << 5) + (blue5 >> 0)) & 0xFF; // second part of 16 bits color

  writeCOMMAND(command, 3);
}

void
uLCD_4DGL::textbackground_color(int color) // set screen background color
{
  char command[3] = ""; // input color is in 24bits like 0xRRGGBB
...
void
uLCD_4DGL::display_control(char mode) // set screen mode to value
{
  char command[3] = "";
...
void
uLCD_4DGL::display_power(char mode) // set screen mode to value
{
  char command[3] = "";
...
void
uLCD_4DGL::set_volume(char value) // set sound volume to value
{
  char command[2] = "";
